...
def setup_logging():
    """Setup logging for auto loans explanations"""
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(f'VFLClientModels/logs/auto_loans_explanations_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
            logging.StreamHandler()
        ]
    )
    return logging.getLogger(__name__)

class AutoLoansFeatureExplainer:
    """
    Auto Loans Feature Importance Explainer
    
    Provides privacy-preserving explanations for auto loan amount predictions
    ...
    def __init__(self, feature_names, random_state=42):
        ...
        self.logger.info("Initializing Auto Loans Feature Importance Explainer...")
        self.logger.info(f"   - Feature count: {len(feature_names)}")
        self.logger.info(f"   - Model type: Regression (loan amount prediction)")
    ...
    def initialize_explainer(self, X_train_sample, num_samples=100):
        ...
        # Load the real auto loans model
        try:
            if TF_AVAILABLE:
                self.model = keras.models.load_model(os.path.join('VFLClientModels', 'saved_models', 'auto_loans_model.keras'))
                self.logger.info("Auto loans model loaded successfully for SHAP explanations.")
            else:
                self.logger.warning("TensorFlow/Keras not available. SHAP explanations will be skipped.")
                self.model = None
        except Exception as e:
            self.logger.error(f"Failed to load auto loans model: {str(e)}")
            self.model = None
    ...
    def explain_single_customer(self, customer_data, customer_id, num_features=10):
        """Explain a single customer's loan prediction - SHAP if available, fallback otherwise"""
        try:
            ...
            if self.model is not None:
                ...
                predicted_log = self.model.predict(customer_data_reshaped)[0][0]
                predicted_amount = float(np.expm1(predicted_log))  # Convert from log scale and ensure float
                self.logger.info(f"   Prediction successful using auto loans model")
            else:
                # Mock prediction for demonstration
                predicted_amount = float(np.random.uniform(5000, 50000))  # Random loan amount, ensure float
    ...
    def print_explanation_report(self, explanation, detailed=True):
        ...
        if detailed:
            ...
            # Risk assessment
            self.logger.info(f"\nRISK ASSESSMENT:")
            if float(explanation['predicted_amount']) > 50000:
                self.logger.info(f"   Risk Level: High (High loan amount)")
            else:
                self.logger.info(f"   Risk Level: Low (Low loan amount)")
    ...
    def explain_multiple_customers(self, customer_data_list, customer_ids, num_features=10):
        ...
        self.logger.info(f"Generating SHAP explanations for {len(customer_ids)} customer.")
...
def main():
    """
    Main function to run auto loans feature importance explanations
    ...
    logger.info("Starting Auto Loans Feature Explanation.")
    logger.info("Mode: Print and log to disk")
    logger.info("=" * 60)
    logger.info("AUTO LOANS FEATURE EXPLANATIONS - LOGGING MODE")
    logger.info("=" * 80)
    ...
    try:
        ...
        logger.info(f"   - Model type: Regression (loan amount prediction)")
        ...
        feature_names = [
            ...
            'investment_balance', 'auto_loan_balance', 'mortgage_balance'
        ]
    ...
    feature_explainer = AutoLoansFeatureExplainer(

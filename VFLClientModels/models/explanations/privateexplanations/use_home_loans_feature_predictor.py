#!/usr/bin/env python3
"""
Home Loans Feature Predictor Usage Script
...
Usage:
    python use_home_loans_feature_predictor.py
"""
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(f'logs/home_loans_feature_predictor_usage_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
def create_sample_intermediate_representations():
    ...
    for i in range(5):
        ...
        # These would normally come from the home loans model's penultimate layer
...
def demonstrate_feature_prediction():
    ...
    logger.info("🚀 HOME LOANS FEATURE PREDICTOR DEMONSTRATION")
    logger.info("=" * 80)
...
def predict_from_dataset_samples():
    ...
    try:
        ...
        for i, idx in enumerate(sample_indices, 1):
            ...
            # Print original intermediate representation values
            logger.info("Original 16D Intermediate Representation:")
            for j, val in enumerate(intermediate_repr[0]):
                logger.info(f"   Dim {j+1:2d}: {val:.6f}")
            logger.info("")
...
def main():
    ...
    logger.info("🚀 HOME LOANS FEATURE PREDICTOR USAGE DEMONSTRATION")
    logger.info("=" * 80)
